...
export const downloadSitePdf = async (req, res) => {
  try {
    ...
    const table = {
      ...
      rows: rows.map((row) => [count++, row.id, row.name, row.siteType, row.address]),
    };
  ...
  try {
    ...
    if (name && siteType) {
      ...
      if (prevSiteType !== siteType) {
        const [rows1] = await pool.query(
          `INSERT INTO crudLogs (userId, action, tableName, columnName, entryId, oldData, newData) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [userId, "update", "sites", "siteType", req.params.id, prevSiteType, siteType]
        );
      }
      if (prevAddress !== address) {
        const [rows1] = await pool.query(
          `INSERT INTO crudLogs (userId, action, tableName, columnName, entryId, oldData, newData) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [userId, "update", "sites", "address", req.params.id, prevAddress, address]
        );
      }
